...
def secure_bookmark(endpoint_func):
    def outer_function_with_security_check(self):
    ...
    return outer_function_with_security_check

def check_ownership_of_bookmark(endpoint_func):
    def outer_function(self, id):
...
def is_valid_post_id_int(endpoint_function):
...
def is_valid_id_int(endpoint_function):
...
def valid_post_params(endpoint_func):
    def outer_function_w_security(self):
        try:
            body = request.get_json()
            ...
            response_obj = {
                'message': 'Make sure your image_url, caption, or alt_text is a string'
            }
            return Response(json.dumps(response_obj), mimetype="application/json", status=400)
        return endpoint_func(self)
    return outer_function_w_security


def valid_patch_params(endpoint_func):
    def outer_function_w_security(self, id):
        try:
            body = request.get_json()
            image_url = body.get('image_url')
            caption = body.get('caption')
...
def valid_text(endpoint_func):
    def outer_function_w_security(self):
        try:
            ...
            response_obj = {
                'message': 'Invalid text={0}'.format(text)
            }
            return Response(json.dumps(response_obj), mimetype="application/json", status=400)
        return endpoint_func(self)
    return outer_function_w_security
...
def secure_post(endpoint_func):
    def outer_function_with_security_check(self, post_id):
        if can_view_post(post_id, self.current_user):
            return endpoint_func(self, post_id)
        else:
            response_obj = {
                'message': 'You don\'t have access to post_id={0}'.format(post_id)
            }
...
def is_valid_like_post_id(endpoint_function):
...
def valid_following_params(endpoint_func):
    def outer_function_w_security(self):
        try:
            body = request.get_json()
            ...
            response_obj = {
                'message': 'Make sure your user exists'
            }
            return Response(json.dumps(response_obj), mimetype="application/json", status=400)
        return endpoint_func(self)
    return outer_function_w_security
...
def is_valid_int_2x(endpoint_function):
